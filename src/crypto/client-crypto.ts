...
async function deriveKey(
  ...
  const ikm = await crypto.subtle.importKey('raw', ENCODER.encode(secretKey), 'HKDF', false, [
    'deriveKey'
  ])
...
async function deriveDomainKey(domainKey: string, context: string): Promise<CryptoKey> {
  const ikm = await crypto.subtle.importKey('raw', ENCODER.encode(domainKey), 'HKDF', false, [
    'deriveKey'
  ])
