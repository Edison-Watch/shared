...
function computePosition(trigger: DOMRect, placement: Placement): { top: number; left: number } {
...
export default function Tooltip({ content, placement = 'top', className, children }: TooltipProps) {
  ...
  const updatePosition = useCallback(() => {
    ...
    rafRef.current = requestAnimationFrame(() => {
      ...
      } else if (placement === 'bottom' && rect.bottom > window.innerHeight - margin) {
        flipped = 'top'
        top = trigger.top - margin
      } else if (placement === 'left' && rect.left < margin) {
        flipped = 'right'
        left = trigger.right + margin
      } else if (placement === 'right' && rect.right > window.innerWidth - margin) {
        flipped = 'left'
  ...
  if (isValidElement<Record<string, unknown>>(children)) {
    ...
    const merged = existing ? `${existing} ${tooltipId.current}` : tooltipId.current
